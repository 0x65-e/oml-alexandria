...
import { isAnnotatedElement, isAnnotationPropertyReference, isAspectReference, isConceptReference, isEnumeratedScalarReference, isFacetedScalarReference, isMember, isRelationEntity, isRelationEntityReference, isScalarPropertyReference, isSpecializableTerm, isSpecializableTermReference, isStructuredPropertyReference, isStructureReference, Ontology } from "./generated/ast";
    ...
    protected getAstNodeHoverContent(node: AstNode): Hover | undefined {
        ...
        if (isAnnotatedElement(node) && node.ownedAnnotations) {
            ...
            node.ownedAnnotations.forEach((annotation) => {
                valString += `* **${annotation.property.$refText}**: ${annotation.value?annotation.value.value:annotation.referenceValue?annotation.referenceValue.$refText:''}\n`
            })
        }
        valString += '\n\n'
        ...
        if (isMember(node)) {
            ...
            if (isSpecializableTerm(node)) {
                if (node.ownedSpecializations) 
                    node.ownedSpecializations.forEach(spec => valString += `\tspecializes: ${spec.specializedTerm.$refText}\n`);
        ...
        } else if (isSpecializableTermReference(node)) { //technically never shows hover for now cause no named node
            if (isFacetedScalarReference(node) || isEnumeratedScalarReference(node)) {
                if (node.scalar.ref) valString += `name: ${node.scalar.ref.name}\n`
            } else if (isScalarPropertyReference(node) || isStructuredPropertyReference(node) || isAnnotationPropertyReference(node)) {
