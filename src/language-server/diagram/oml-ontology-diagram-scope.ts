import { AstNode, isReference, streamAllContents } from "langium";
import {
  Aspect,
  Assertion,
  Axiom,
  ...
  implements OmlOntologyDiagramScope
{
  ...
  private readonly relationEntities: Map<RelationEntity, Set<AstNode>> =
    new Map();
  private readonly relationIncidentElements: Map<RelationEntity, Set<AstNode>> =
    new Map();
  ...
  private readonly structureAssertions: Map<StructureInstance, Set<Assertion>> =
    new Map();
  ...
  private includes(e: AstNode | undefined): boolean {
    ...
    else if (isSpecializationAxiom(e))
      return (
  ...
  private analyzeOntology(ontology: Ontology | undefined): void {
    if (ontology != undefined) {
      ...
      streamAllContents(ontology).forEach((node) => {
        if (
          isAxiom(node) ||
          isType(node) ||
          isSemanticProperty(node) ||
          isNamedInstance(node) ||
          isAssertion(node)
        ) {
          ...
          this.allImportedElements.add(node);
        }
      });
    }
  }
  ...
  private phase2AddClassifierScalarProperty(
    cls: Classifier,
    p: ScalarProperty
  ): void {
  ...
  private phase2AddClassifierStructuredProperty(
    cls: Classifier,
    p: StructuredProperty
  ): void {
  ...
  private phase2FilterEntityAxioms(e: Entity, others: Set<AstNode>): void {
    ...
    others.forEach((o) => {
      if (isKeyAxiom(o)) {
        ...
        isScalarPropertyRestrictionAxiom(o) ||
        isStructuredPropertyRestrictionAxiom(o)
      ) {
        ...
        if (
          o.relation.ref &&
          this.includes(o.relation.ref.$container) &&
          this.includes(o.range.ref)
        ) {
          ax.add(o);
        }
  ...
  private phase1ScanInstanceAssertions(
    i: NamedInstance,
    others: Set<AstNode>
  ): void {
    ...
    if (isAspect(element)) {
      ...
      switch (this.mode) {
        case Mode.Phase1:
          ...
          const others: Set<AstNode> = this.relationEntities.get(
            element
          ) as Set<AstNode>;
          const incident: Set<AstNode> = this.relationIncidentElements.get(
            element
          ) as Set<AstNode>;
          if (others) this.phase1ScanEntityAxioms(element, others);
        ...
        case Mode.Phase2:
          this.phase2ScanAllClassifierProperties(element);
          const others2: Set<AstNode> | undefined =
            this.relationEntities.get(element);
          if (others2) this.phase2FilterEntityAxioms(element, others2);
          break;
      }
      ...
      switch (this.mode) {
        case Mode.Phase1:
          ...
          const others: Set<AstNode> | undefined =
            this.instanceAssertions.get(element);
      ...
      isConceptInstanceReference(element) ||
      isRelationInstanceReference(element)
    ) {
