...
import {
  ...
  OmlEdge_SpecializationArrowEdgeView,
  OmlEditableLabel_SLabelView,
  OmlNode_ClassNodeView,
  SButton_ExpandButtonView,
...
/*       (node.children ?? [])[0].children?.push(<SShapeElement>{
  ...
  createConceptInstanceNode(ci: ConceptInstance): OmlNode {
    const text: string =
      this.getLocalName(ci) +
      ": " +
      ci.ownedTypes.map((a) => getAbbreviatedIriRef(a.type)).join(",");
  ...
  createRelationInstanceNode(
    ri: RelationInstance,
    from: SModelElement,
    to: SModelElement
  ): OmlNode {
    const text: string =
      this.getLocalName(ri) +
      ": " +
      ri.ownedTypes.map((a) => getAbbreviatedIriRef(a.type)).join(",");
  ...
  getPropertyCompartment(element: SModelElement): SCompartment | undefined {
    if (element.children == undefined) return undefined;
    return element.children.find((it) =>
      it.id.endsWith(".property.compartment")
    );
  }
  ...
  getAntecedentCompartment(element: SModelElement): SCompartment | undefined {
    if (element.children == undefined) return undefined;
    return element.children.find((it) =>
      it.id.endsWith(".antecendent.compartment")
    );
  }
  ...
  getConsequentCompartment(element: SModelElement): SCompartment | undefined {
    if (element.children == undefined) return undefined;
    return element.children.find((it) =>
      it.id.endsWith(".consequent.compartment")
    );
  }
  ...
  createScalarPropertyLabel(cls: Classifier, property: ScalarProperty): SLabel {
    return {
      id: this.idCache.uniqueId(
        cls.name + ".scalar." + property.name,
        property
      ),
  ...
  createKeyAxiomLabel(entity: Entity, ax: KeyAxiom): SLabel {
    return {
      text:
        "key: " +
        ax.properties
          .map(({ ref }) => (ref ? getAbbreviatedIri(ref) : ""))
          .join(", "),
      type: SLabel_SLabelView_text,
      id: this.idCache.uniqueId(
  ...
  createRangeRestrictionAxiomLabel(
    ...
    return {
      id: this.idCache.uniqueId(
        entity.name + ".rangeRestriction." + ax.property.ref?.name,
        ax
      ),
      type: SLabel_SLabelView_text,
  ...
  createCardinalityRestrictionAxiomLabel(
    ...
    return {
      id: this.idCache.uniqueId(
        entity.name + ".cardinalityRestriction." + ax.property.ref?.name
      ),
      type: SLabel_SLabelView_text,
      text:
        "⎸" +
  ...
  createValueRestrictionAxiomLabel(
    ...
    return {
      id: this.idCache.uniqueId(
        entity.name + ".valueRestriction." + ax.property.ref?.name,
        ax
      ),
      type: SLabel_SLabelView_text,
      text:
        ax.property.ref?.name +
        " = " +
        (isScalarPropertyValueRestrictionAxiom(ax)
          ? ax.value.value.toString()
          : ax.value.type?.ref?.name),
    };
  }
  ...
  createValueAssertionLabel(
    i: NamedInstance,
    ax: ScalarPropertyValueAssertion
  ): SLabel {
    return {
      id: this.idCache.uniqueId(
        i.name + ".valueAssertion." + ax.property.ref?.name,
        ax
      ),
      type: SLabel_SLabelView_text,
  ...
  createSpecializationAxiomEdge(
    ...
    return {
      ...
      type: OmlEdge_SpecializationArrowEdgeView,
      sourceId: from.id,
      targetId: to.id,
    };
  }
  ...
  createRelationEntityEdge(
    entity: RelationEntity,
    from: SModelElement,
    to: SModelElement
  ): SEdge {
    const forwardRelationName = entity.forwardRelation?.name;
    if (!forwardRelationName) throw new Error("Missing forward relation!");
    const id: string = this.idCache.uniqueId(forwardRelationName, entity);
    return {
      id: id,
  ...
  createRelationCardinalityRestrictionAxiomEdge(
    ...
    const id: string = this.idCache.uniqueId(
      from.id + ".restrictsCardinality." + axiom.relation.ref?.name,
      axiom
    );
    let notation: string;
    switch (axiom.kind) {
    ...
    return {
      ...
      children: [
        <SLabel>{
          ...
          text:
            "⎸" +
            axiom.relation.ref?.name +
            "⎹" +
            notation +
            axiom.cardinality.value.toString(),
        },
      ],
    };
  }
  ...
  createRelationRangeRestrictionAxiomEdge(
    ...
    const id: string = this.idCache.uniqueId(
      from.id + ".restrictsRange." + axiom.relation.ref?.name,
      axiom
    );
    return {
      id: id,
  ...
  createRelationTargetRestrictionAxiomEdge(
    ...
    const id: string = this.idCache.uniqueId(
      from.id + ".restrictsTarget." + axiom.relation.ref?.name,
      axiom
    );
    return {
      id: id,
  ...
  createStructuredPropertyEdge(
    ...
    const id: string = this.idCache.uniqueId(
      cls.name + ".structured-property." + property.name,
      property
    );
    return {
      id: id,
  ...
  getLocalName(element: AstNode): String {
    if (isOntology(element)) {
      return getPrefixIn(element, this.ontology) ?? "";
    } else if (isMember(element)) {
      return getAbbreviatedIriIn(element, this.ontology) ?? "";
    } else if (isReference(element)) {
      return getAbbreviatedIriRef(element) ?? "";
    } else
      throw new Error("No local name for element of kind: " + element.$type);
  }
}
